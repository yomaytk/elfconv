...
## elfconvs Architecture Overview
...
- Execute a ***semantics function*** that operates equivalent to the machine instruction, using the retrieved values.
...
When we add the new instruction support, we should add the implementation to test the added instruction. We use the testing methods provided by Remill as they are.
🚧
## Debugging
During elfconv’s development, it’s fine if compile‑time errors are caught, but there are often cases where the converted executable’s runtime results are slightly off. Debugging such inexplicable behavior at runtime is often hard, so elfconv includes implementations designed to help with that debugging.
🚧

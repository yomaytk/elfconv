...

{
  ...
  do {
    ...
    // don't use stdin for wasm on browser
    int array_size;
    array_size = 100;
    printf("Array size: %d\n", array_size);
    arsize = array_size;
          ...
          for (j = kp1; j < n; j++) {
            ...
            // in!!! daxpy_ur
            {
              ...
              if (1 != 1 || 1 != 1) {
                ...
                for (i = 0; i < n - (k + 1); i++) {
                  (&a[lda * j + k + 1])[iy] =
                      (&a[lda * j + k + 1])[iy] + t * (&a[lda * k + k + 1])[ix];
                  ix = ix + 1;
                  iy = iy + 1;
                }
                return;
              ...
              if (m != 0) {
                for (i = 0; i < m; i++)
                  (&a[lda * j + k + 1])[i] =
                      (&a[lda * j + k + 1])[i] + t * (&a[lda * k + k + 1])[i];
                if (n - (k + 1) < 4)
                  return;
              }
              for (i = m; i < n - (k + 1); i = i + 4) {
                (&a[lda * j + k + 1])[i] = (&a[lda * j + k + 1])[i] + t * (&a[lda * k + k + 1])[i];
                (&a[lda * j + k + 1])[i + 1] =
                    (&a[lda * j + k + 1])[i + 1] + t * (&a[lda * k + k + 1])[i + 1];
                (&a[lda * j + k + 1])[i + 2] =
                    (&a[lda * j + k + 1])[i + 2] + t * (&a[lda * k + k + 1])[i + 2];
                (&a[lda * j + k + 1])[i + 3] =
                    (&a[lda * j + k + 1])[i + 3] + t * (&a[lda * k + k + 1])[i + 3];
              }
              // }
            }
          }
    ...
    if (job == 0) {
          ...
          // in!!! daxpy_ur
          {
            ...
            if (1 != 1 || 1 != 1) {
              ...
              if (1 < 0)
                iy = (-(n - (k + 1)) + 1) * 1 + 1;
              for (i = 0; i < n - (k + 1); i++) {
                (&b[k + 1])[iy] = (&b[k + 1])[iy] + t * (&a[lda * k + k + 1])[ix];
                ix = ix + 1;
                iy = iy + 1;
              }
              return;
            ...
            if (m != 0) {
              for (i = 0; i < m; i++)
                (&b[k + 1])[i] = (&b[k + 1])[i] + t * (&a[lda * k + k + 1])[i];
              if (n - (k + 1) < 4)
                return;
            }
            for (i = m; i < n - (k + 1); i = i + 4) {
              (&b[k + 1])[i] = (&b[k + 1])[i] + t * (&a[lda * k + k + 1])[i];
              (&b[k + 1])[i + 1] = (&b[k + 1])[i + 1] + t * (&a[lda * k + k + 1])[i + 1];
              (&b[k + 1])[i + 2] = (&b[k + 1])[i + 2] + t * (&a[lda * k + k + 1])[i + 2];
              (&b[k + 1])[i + 3] = (&b[k + 1])[i + 3] + t * (&a[lda * k + k + 1])[i + 3];
            }
            // }
          }
        }
      ...
      for (kb = 0; kb < n; kb++) {
        ...
        // in!!! daxpy_ur
        {
          ...
          if (m != 0) {
            for (i = 0; i < m; i++)
              (&b[0])[i] = (&b[0])[i] + t * (&a[lda * k + 0])[i];
            if (k < 4)
              return;
          }
          for (i = m; i < k; i = i + 4) {
            (&b[0])[i] = (&b[0])[i] + t * (&a[lda * k + 0])[i];
            (&b[0])[i + 1] = (&b[0])[i + 1] + t * (&a[lda * k + 0])[i + 1];
            (&b[0])[i + 2] = (&b[0])[i + 2] + t * (&a[lda * k + 0])[i + 2];
            (&b[0])[i + 3] = (&b[0])[i + 3] + t * (&a[lda * k + 0])[i + 3];
          }
          // }
        }
      }

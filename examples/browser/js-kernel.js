var Module = (() => {
  return (async function (moduleArg = {}) {
    ...
    function newProcess(path, isForked, sDataSrcP, sDataLen, mBytesSrcP, mBytesLen, parEcvPid) {
      ...
      let processWorker = new Worker(new URL(path, import.meta.url), {
        type: "module",
        name: `process-worker-${newEcvPId}`
      });

      processes.set(newEcvPId, {
      ...
      if (isForked) {
        copySData = e => {
          ...
          parWMemory32[sDataSrcP + (sDataLen - 8) >> 2] = newEcvPId;
      ...
      processWorker.onmessage = e => {
        ...
        if (d.cmd === "sysRun") {
          ...
          if (!isForked) {
            throw new Error(`memory copy request must not be sent to init process (ecvPid ${newEcvPId}).`);
          }
    ...
    function ___syscall_clone(sDataP, sDataPLen, mBytes, mBytesLen, copyFinBell, parEcvPid) {
      try {
        let child_pid = newProcess(initProcessJsPath, true, sDataP, sDataPLen, mBytes, mBytesLen, copyFinBell, parEcvPid);
        return child_pid;
      } catch (e) {
    ...
    function ___syscall_exit(ecvPid, code) {
      ...
      try {
        ...
        if (thisEcvPr.parent) {
          ...
          // console.log(`0: ${Atomics.load(monitorView, 0)}, 1: ${Atomics.load(monitorView, 1)}, 2: ${Atomics.load(monitorView, 2)}, 3: ${Atomics.load(monitorView, 3)}`);

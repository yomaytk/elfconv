var Module = (() => {
  return (async function (moduleArg = {}) {
    ...
    function newProcess(path, isForked, sDataSrcP, sDataLen, mBytesSrcP, mBytesLen, parWorkerId) {
      ...
      if (isForked) {
        copySData = e => {
          let d = e["data"];
          // copy `memory_arena_bytes` and `shared_data`.
          let parWMemory8_1 = new Uint8Array(processes.get(parWorkerId).wMemory.buffer);
          (growMemViews(newWMemory), HEAPU8).set(parWMemory8_1.subarray(mBytesSrcP, mBytesSrcP + mBytesLen), d.mBytesDstP);
          let parWMemory8_2 = new Uint8Array(processes.get(parWorkerId).wMemory.buffer);
      ...
      processWorker.onmessage = e => {
        ...
        if (d.cmd === "sysRun") {
          ...
          if (!isForked) {
            throw new Error(`memory copy request must not be sent to init process (workerId ${newWorkerId}).`);
          }
          // copy the parent process state to the new process state.
          copySData(e);
          // notify to parent process worker.
          let parBellView = new Int32Array(processes.get(parWorkerId).copyFinBell);
    ...
    function ___syscall_clone(sDataP, sDataPLen, mBytes, mBytesLen, copyFinBell, parWorkerId) {
      try {
        newProcess(initProcessJsPath, true, sDataP, sDataPLen, mBytes, mBytesLen, copyFinBell, parWorkerId);

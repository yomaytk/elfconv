var Module = (() => {
  return (async function (moduleArg = {}) {
    ...
    var arguments_ = [];
    var initProcessJsPath = "process-worker.js";
    var quit_ = (status, toThrow) => {
      throw toThrow
    };
    var _scriptName = import.meta.url;

    var globalWorkerCounter = 0;
    var gWasmMemory;
    var SyscallPtrMap = new Map();
    var processes = new Map();

    function getNewWorkerId() {
      return globalWorkerCounter++;
    }
    ...
    function newProcess(path, isFork, sDataSrcP, sDataLen, mBytesSrcP, mBytesLen, parHeapBase, parWorkerId) {
      ...
      let newWorkerId = getNewWorkerId();

      // init Worker
      let processWorker = new Worker(new URL(path, import.meta.url), {
        type: "module",
        name: `process-worker-${newWorkerId}`
      });

      processes.set(newWorkerId, {
      ...
      let copyNoteBell = new SharedArrayBuffer(4);
      ...
      // if this process is generated by clone syscall, it is necessary to copy the process state to the Wasm memory.
      if (isFork) {

        // ???????
        // copy emscripte runtime static data.
        // let parWMemory8 = new Uint8Array(processes.get(parWorkerId).wMemory.buffer);
        // (growMemViews(newWMemory), HEAP8).set(parWMemory8.subarray(0, parHeapBase), 0);
        ...
        copySData = e => {
          let d = e["data"];
          let parWMemory8 = new Uint8Array(processes.get(parWorkerId).wMemory.buffer);
          // copy `MemoryArena bytes` and `shared data`.
          (growMemViews(newWMemory), HEAPU8).set(parWMemory8.subarray(mBytesSrcP, mBytesSrcP + mBytesLen), d.mBytesP8);
          (growMemViews(newWMemory), HEAPU8).set(parWMemory8.subarray(sDataSrcP, sDataSrcP + sDataLen), d.sDataP8);
        }
      } else {
        entryCmd = "main";
        copySData = e => { throw e; };
      ...
      processWorker.onmessage = e => {
        ...
        if (d.cmd === "sysRun") {
          ...
          // update global Wasm Memory.
          gWasmMemory = processes.get(d.workerId).wMemory;
          updateMemoryViews(gWasmMemory);

          let headPtr32 = d.spHead32;
          let sysNum = (growMemViews(gWasmMemory), HEAP32)[headPtr32];
          let argsNum = (growMemViews(gWasmMemory), HEAP32)[headPtr32 + 1];

          let sysRvalPtr = headPtr32 + 2 + argsNum;
          let waitPtr = sysRvalPtr + 1;

          let sysArgs = new Int32Array(argsNum + 1);

          for (var i = 0; i < argsNum; i++) {
            sysArgs[i] = (growMemViews(gWasmMemory), HEAP32)[headPtr32 + 2 + i];
          }

          // for clone syscall.
          sysArgs[argsNum] = d.workerId;
          console.log(`this workerId: ${d.workerId}`);

          let tgtKernelFunction = SyscallPtrMap.get(sysNum);
          if (!tgtKernelFunction) {
            throw new Error(`unknown syscall: ${sysNum}`);
          }
          if (tgtKernelFunction.length != argsNum && sysNum != ECV_CLONE) {
          ...
          (growMemViews(gWasmMemory), HEAP32)[sysRvalPtr] = sysRval;

          // notify to process worker
          Atomics.store((growMemViews(gWasmMemory), HEAP32), waitPtr, 1);
          Atomics.notify((growMemViews(gWasmMemory), HEAP32), waitPtr, 1);
        } else if (d.cmd === "initOk") {
          console.log("3");
          // let parWMemory8 = new Uint8Array(processes.get(parWorkerId).wMemory.buffer);
          // copy emscripte runtime static data.
          // (growMemViews(newWMemory), HEAP8).set(parWMemory8.subarray(0, parHeapBase), 0);
        ...
        } else if (d.cmd === "giveSDataP") {
          copySData(e);
          console.log("5");
          // notify copySData finishing.
          // let bellView = new Int32Array(copyNoteBell);
          // Atomics.store(bellView, 0, 1);
          // Atomics.notify(bellView, 0, 1);
          processWorker.postMessage({
            cmd: "startWorker",
            entry: entryCmd,
          });
    ...
    function ___syscall_clone(sDataP, sDataPLen, mBytes, mBytesLen, parHeapBase, parWorkerId) {
      try {
        newProcess(initProcessJsPath, true, sDataP, sDataPLen, mBytes, mBytesLen, parHeapBase, parWorkerId);
        return 100; // (FIXME) 100 for simplicity.
      } catch (e) {
    ...
    MEMFS.doesNotExistError.stack = "<generic error, no stack>";
    {
      ...
      // if (Module["initProcessJsPath"]) initProcessJsPath = Module["initProcessJsPath"]
    }
    ...
    // args.unshift(initProcessJsPath);

...
const static std::unordered_map<std::string,
                                std::function<test_runner::RegisterValueRef(AArch32State &)>>
    reg_to_accessor = {
        {"r15",
         [](AArch32State &st) -> test_runner::RegisterValueRef { return &st.gpr.r15.dword; }},
        {"sp", [](AArch32State &st) -> test_runner::RegisterValueRef { return &st.gpr.r13.dword; }},
        {"r1", [](AArch32State &st) -> test_runner::RegisterValueRef { return &st.gpr.r1.dword; }},
        {"z", [](AArch32State &st) -> test_runner::RegisterValueRef { return &st.sr.z; }}};
...
class TestOutputSpec {
 ...
 private:
  ...
  void ApplyCondition(AArch32State &state, std::string reg,
    ...
    std::visit([&](auto arg) { ApplyCondWithAcc(arg, accessor->second, state); }, value);
  }


  template <typename T>
  void CheckRegEq(T value, std::function<test_runner::RegisterValueRef(AArch32State &)> accessor,
                  AArch32State &state) const {
  ...
  void CheckCondition(AArch32State &state, std::string reg,
    ...
    std::visit([&](auto arg) { CheckRegEq(arg, accessor->second, state); }, value);
  }
 ...
 public:
  ...
  void AddPrecWrite(uint64_t addr, T value) {
    this->initial_memory_conditions.push_back(
        [addr, value](test_runner::MemoryHandler &mem_hand) { mem_hand.WriteMemory(addr, value); });
  }
...
class TestSpecRunner {
 ...
 public:
  TestSpecRunner(llvm::LLVMContext &context, remill::ArchName name)
      : lifter(test_runner::LiftingTester(context, remill::OSName::kOSLinux, name)),
        tst_ctr(0),
  ...
  void RunTestSpec(const TestOutputSpec &test) {
    ...
    auto maybe_func = lifter.LiftInstructionFunction(ss.str(), test.target_bytes, test.addr);
...
TEST(ThumbRandomizedLifts, PopPC) {
  ...
  TestOutputSpec spec(0x12, insn_data, remill::Instruction::Category::kCategoryFunctionReturn,
...
TEST(ArmRandomizedLifts, RelPcTest) {
  ...
  TestOutputSpec spec(0x12, insn_data, remill::Instruction::Category::kCategoryNormal,
...
TEST(RegressionTests, RegressionPreffixSuffixInsn) {
  ...
  TestOutputSpec spec(0x00014182, insn_data,
...
TEST(RegressionTests, AARCH64RegSize) {
  ...
  auto lifted = op_lifter->LoadRegValue(&target_lift->getEntryBlock(), st_ptr, "W0");

  CHECK_EQ(lifted->getType()->getIntegerBitWidth(), 32);
  op_lifter->ClearCache();
  auto lifted2 = op_lifter->LoadRegValue(&target_lift->getEntryBlock(), st_ptr, "W0");
...
TEST(ArmContextTests, ThumbBXIndirect) {
  ...
  remill::Instruction::InstructionFlowCategory jmp =
      remill::Instruction::IndirectJump(remill::Instruction::IndirectFlow(std::nullopt));


  remill::Instruction::IndirectJump str = std::get<remill::Instruction::IndirectJump>(flow.flows);
...
TEST(ArmContextTests, ThumbMovIgnoresAnyStateChange) {
  ...
  remill::Instruction::InstructionFlowCategory jmp =
      remill::Instruction::IndirectJump(remill::Instruction::IndirectFlow(remill::kThumbContext));
  ...
  remill::Instruction::IndirectJump str2 = std::get<remill::Instruction::IndirectJump>(jmp);
...
TEST(ArmContextTests, ThumbBLXInterProcStaysInSameMode) {
  ...
  remill::Instruction::InstructionFlowCategory jmp = remill::Instruction::DirectFunctionCall(
...
TEST(ArmContextTests, ThumbBLStaysInSameContext) {
  ...
  auto dfcall = std::get<remill::Instruction::DirectFunctionCall>(act_insn.flows);
...
TEST(ArmContextTests, ThumbBPLRegressionTest) {
  ...
  auto act_cond_insn_flow = std::get<remill::Instruction::ConditionalInstruction>(act_insn.flows);

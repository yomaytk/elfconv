...
class SingleInstructionSleighContext {
 private:
  ...
  std::optional<int32_t> oneInstruction(uint64_t address,
...
struct ContextRegMappings {
 ...
 public:
  ContextRegMappings(std::unordered_map<std::string, std::string> context_reg_mapping,
                     std::unordered_map<std::string, size_t> vnode_size_mapping)
  ...
  const std::unordered_map<std::string, std::string> &GetInternalRegMapping() const;
};

class SleighDecoder {
 public:
  SleighDecoder() = delete;
  SleighDecoder(const remill::Arch &arch, std::string sla_name, std::string pspec_name,
  ...
  virtual void InitializeSleighContext(uint64_t address, SingleInstructionSleighContext &,
                                       const ContextValues &) const = 0;


  virtual llvm::Value *LiftPcFromCurrPc(llvm::IRBuilder<> &bldr, llvm::Value *curr_pc,
  ...
  const std::unordered_map<std::string, std::string> &GetStateRegRemappings() const;

  std::shared_ptr<remill::OperandLifter> GetOpLifter() const;

 protected:
  ControlFlowStructureAnalysis::SleighDecodingResult
  DecodeInstructionImpl(uint64_t address, std::string_view instr_bytes, Instruction &inst,

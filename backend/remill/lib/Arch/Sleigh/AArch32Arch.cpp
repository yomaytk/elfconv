...
AArch32Arch::AArch32Arch(llvm::LLVMContext *context_, OSName os_name_, ArchName arch_name_)
...
AArch32Arch::DefaultLifter(const remill::IntrinsicTable &intrinsics_table) const {
...
Arch::ArchPtr Arch::GetAArch32(llvm::LLVMContext *context_, OSName os_name_, ArchName arch_name_) {
...
bool AArch32Arch::DecodeSleigh(uint64_t address, std::string_view instr_bytes, Instruction &inst,
                               DecodingContext context) const {
  return this->thumb_decoder.DecodeInstruction(address, instr_bytes, inst, context);
}

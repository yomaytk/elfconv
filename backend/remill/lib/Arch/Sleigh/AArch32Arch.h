...
class AArch32Arch final : public AArch32ArchBase {
 ...
 private:
  ...
  bool DecodeAArch32(uint64_t address, std::string_view instr_bytes, Instruction &inst,
                     DecodingContext context) const;


  bool DecodeSleigh(uint64_t address, std::string_view instr_bytes, Instruction &inst,
                    DecodingContext context) const;

...
DEF_SEM(RCL, D dst, S1 src1, S2 src2) {
  ...
  auto op_size = BitSizeOf(src1);
  auto count_mask = Select(UCmpEq(op_size, 64), long_mask, short_mask);
  auto count_mod = Select(UCmpLt(op_size, 32), UAdd(op_size, one), UAdd(count_mask, one));
  ...
  if (temp_count) {
    ...
    auto new_val =
        UOr(UOr(UShl(val, temp_count), UShl(carry, USub(temp_count, one))), UShr(right, one));
    WriteZExt(dst, new_val);
...
DEF_SEM(RCR, D dst, S1 src1, S2 src2) {
  ...
  if (temp_count) {
    ...
    auto new_val =
        UOr(UOr(UShr(left, one), UShl(carry, USub(op_size, temp_count))), UShl(right, one));
    WriteZExt(dst, new_val);

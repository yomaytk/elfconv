...
//   auto take_branch = __remill_compare_sgt(BAnd(BNot(FLAG_ZF), BXnor(FLAG_SF, FLAG_OF)));
...
DEF_SEM_U64_STATE(JNZ, PC taken, PC not_taken) {
  auto taken_branch = __remill_compare_neq(BNot(FLAG_ZF));
  return taken_branch;
}

...
DEF_SEM(LGDT, M32 src) {
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kAssertPrivileged);
  state.addr_to_load = AddressOf(src);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kX86LoadGlobalDescriptorTable);
}

DEF_SEM(LIDT, M32 src) {
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kAssertPrivileged);
  state.addr_to_load = AddressOf(src);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kX86LoadInterruptDescriptorTable);
}

DEF_SEM(DoRDMSR) {
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kAssertPrivileged);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kX86ReadModelSpecificRegister);
}

DEF_SEM(DoWRMSR) {
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kAssertPrivileged);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kX86WriteModelSpecificRegister);
}

DEF_SEM(DoWBINVD) {
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kAssertPrivileged);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kX86WriteBackInvalidate);
}
...
DEF_SEM(WRITE_CONTROL_REG_32, R64W dst, R32 src) {
  ...
  Write(FLAG_PF, u);
  Write(FLAG_CF, u);
  __remill_sync_hyper_call(state, rt_m, kSetCR);
}
...
DEF_SEM(WRITE_DEBUG_REG_32, R64W dst, R32 src) {
  ...
  Write(FLAG_PF, u);
  Write(FLAG_CF, u);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kX86SetDebugReg);
}
...
DEF_SEM(WRITE_DEBUG_REG_64, R64W dst, R64 src) {
  ...
  Write(FLAG_PF, u);
  Write(FLAG_CF, u);
  __remill_sync_hyper_call(state, rt_m, SyncHyperCall::kAMD64SetDebugReg);
}
#endif
}  // namespace

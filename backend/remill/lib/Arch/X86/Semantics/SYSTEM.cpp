...
DEF_SEM(LGDT, M32 src) {
  ...
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kX86LoadGlobalDescriptorTable);
}

DEF_SEM(LIDT, M32 src) {
  memory = __remill_sync_hyper_call(state, memory, SyncHyperCall::kAssertPrivileged);
  state.addr_to_load = AddressOf(src);
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kX86LoadInterruptDescriptorTable);
}

DEF_SEM(DoRDMSR) {
  memory = __remill_sync_hyper_call(state, memory, SyncHyperCall::kAssertPrivileged);
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kX86ReadModelSpecificRegister);
}

DEF_SEM(DoWRMSR) {
  memory = __remill_sync_hyper_call(state, memory, SyncHyperCall::kAssertPrivileged);
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kX86WriteModelSpecificRegister);
}

DEF_SEM(DoWBINVD) {
  memory = __remill_sync_hyper_call(state, memory, SyncHyperCall::kAssertPrivileged);
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kX86WriteBackInvalidate);
}
...
DEF_SEM(WRITE_DEBUG_REG_32, R64W dst, R32 src) {
  ...
  Write(FLAG_PF, u);
  Write(FLAG_CF, u);
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kX86SetDebugReg);
}
...
DEF_SEM(WRITE_DEBUG_REG_64, R64W dst, R64 src) {
  ...
  Write(FLAG_PF, u);
  Write(FLAG_CF, u);
  return __remill_sync_hyper_call(state, memory, SyncHyperCall::kAMD64SetDebugReg);
}
#endif
}  // namespace

...
DEF_SEM(PUSH, S1 src1) {
  PushToStack(rt_m, state, Read(src1));
}
...
DEF_SEM(DoPUSHA) {
  ...
  PushToStack<uint16_t>(rt_m, state, Read(REG_AX));
  PushToStack<uint16_t>(rt_m, state, Read(REG_CX));
  PushToStack<uint16_t>(rt_m, state, Read(REG_DX));
  PushToStack<uint16_t>(rt_m, state, Read(REG_BX));
  PushToStack<uint16_t>(rt_m, state, sp);
  PushToStack<uint16_t>(rt_m, state, Read(REG_BP));
  PushToStack<uint16_t>(rt_m, state, Read(REG_SI));
  PushToStack<uint16_t>(rt_m, state, Read(REG_DI));
}

DEF_SEM(DoPUSHAD) {
  uint32_t esp = Read(REG_ESP);
  PushToStack<uint32_t>(rt_m, state, Read(REG_EAX));
  PushToStack<uint32_t>(rt_m, state, Read(REG_ECX));
  PushToStack<uint32_t>(rt_m, state, Read(REG_EDX));
  PushToStack<uint32_t>(rt_m, state, Read(REG_EBX));
  PushToStack<uint32_t>(rt_m, state, esp);
  PushToStack<uint32_t>(rt_m, state, Read(REG_EBP));
  PushToStack<uint32_t>(rt_m, state, Read(REG_ESI));
  PushToStack<uint32_t>(rt_m, state, Read(REG_EDI));
}
#endif

DEF_SEM(DoPUSHF) {
  SerializeFlags(state);
  PushToStack<uint16_t>(rt_m, state, TruncTo<uint16_t>(state.rflag.flat));
}

#if 32 == ADDRESS_SIZE_BITS
DEF_SEM(DoPUSHFD) {
  SerializeFlags(state);
  PushToStack<uint32_t>(rt_m, state, TruncTo<uint32_t>(state.rflag.flat));
}
#else
DEF_SEM(DoPUSHFQ) {
  SerializeFlags(state);
  PushToStack<uint64_t>(rt_m, state, state.rflag.flat);
}

...
DEF_SEM_T_RUN(PUSH, S1 src1, R64 rsp) {
  auto val = Read(src1);
  addr_t op_size = ZExtTo<addr_t>(ByteSizeOf(src1));
  addr_t old_sp = Read(rsp);
  addr_t new_sp = USub(old_sp, op_size);
  MWriteZExt(WritePtr<addr_t>(new_sp), val);
  return new_sp;
}

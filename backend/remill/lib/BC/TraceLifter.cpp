...
bool TraceLifter::Impl::Lift(uint64_t addr, const char *fn_name,
  ...
  if (std::string(fn_name).ends_with("_noopt")) {
    not_vr_opt = true;
  }
  ...
  auto get_opt_func_trace_decl_ = [=](uint64_t trace_addr) -> llvm::Function * {
    if (!manager.isFunctionEntry(trace_addr)) {
      return nullptr;
    }
...
/* insert debug call stack function (for debug) */
  ...
  inst_lifting_start:
      ...
      switch (inst.category) {
        ...
        case Instruction::kCategoryDirectJump:
          ...
          if (!manager.isWithinFunction(trace_addr, inst.branch_taken_pc)) {
            auto callee_def_func = get_opt_func_trace_decl_(inst.branch_taken_pc);
            if (callee_def_func) {
        ...
        direct_func_call:
          try_add_delay_slot(true, block);
          auto target_trace = get_opt_func_trace_decl_(inst.branch_taken_pc);
    ...
    if (!lift_all_insn && (indirectbr_block || not_vr_opt)) {
    ...
    // if not_vr_opt is set, the first instruction may be the instruction on the way.
    ...
    // if the lifted function is `indirectbr_block` or `not_vr_opt`, we must store the return values of semantics functions to the registers.
...
void TraceLifter::Impl::Optimize() {
  ...
  for (auto lifted_func : opt_target_funcs) {
    std::cout << "\r["
              ...
              << "\033[0m"
              << "]"
              << " Opt Pass 1: [" << opt_cnt << "/" << opt_target_funcs.size() << "]" << std::flush;
  ...
  for (auto lifted_func : opt_target_funcs) {
    std::cout << "\r["
              ...
              << "\033[0m"
              << "]"
              << " Opt Pass 2: [" << opt_cnt2 << "/" << opt_target_funcs.size() << "]"
              << std::flush;
    auto virtual_regs_opt = VirtualRegsOpt::func_v_r_opt_map[lifted_func];

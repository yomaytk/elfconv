...
/* insert debug call stack function (for debug) */
  ...
  inst_lifting_start:
    ...
    // auto pa2 = func_instaddr_set_map.at(trace_addr);
    printf("trace_addr: 0x%lx, end_addr: 0x%lx, diff: %ld\n", trace_addr, *addrset_in_func.begin(),
           *addrset_in_func.begin() - trace_addr);
    fflush(stdout);
  }

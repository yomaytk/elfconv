...
const std::string EntrypointFunction::metadata_value = BaseFunction::metadata_value + ".entrypoint";

const std::string ExternalFunction::metadata_value = BaseFunction::metadata_value + ".external";

const std::string AbiLibraries::metadata_value = ExternalFunction::metadata_value + ".abilibraries";

const std::string CFGExternal::metadata_value = ExternalFunction::metadata_value + ".cfgexternal";

const std::string ExtWrapper::metadata_value = ExternalFunction::metadata_value + ".extwrapper";


const std::string Helper::metadata_value = BaseFunction::metadata_value + ".helper";


const std::string RemillHelper::metadata_value = Helper::metadata_value + ".remill";

const std::string McSemaHelper::metadata_value = Helper::metadata_value + ".mcsema";

const std::string Semantics::metadata_value = Helper::metadata_value + ".semantics";

llvm::MDNode *TieFunction(llvm::Function *first, llvm::Function *second, const std::string &kind) {
...
TieFunctions(llvm::Function *first, llvm::Function *second, const std::string &kind) {
  LOG_IF(FATAL, IsTied(first, kind) || IsTied(second, kind))
      << "Tried to tie already tied functions " << first->getName().str() << " to "
      << second->getName().str();

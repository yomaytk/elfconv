...
#define DEF_SEM_V128U64(name, ...) \
  ALWAYS_INLINE __attribute__((flatten)) static V128U64 name(##__VA_ARGS__)

#define DEF_SEM_V128U64_STATE(name, ...) \
  ALWAYS_INLINE __attribute__((flatten)) static V128U64 name(State &state, ##__VA_ARGS__)

#define DEF_SEM_V128U64_RUN(name, ...) \
  ALWAYS_INLINE __attribute__((flatten)) static V128U64 name(RuntimeManager *runtime_manager, \
                                                             ##__VA_ARGS__)

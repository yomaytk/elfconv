...
// ALWAYS_INLINE static uint128_t __remill_read_memory_128(RuntimeManager *rt_m, addr_t addr);

// ALWAYS_INLINE static RuntimeManager *__remill_write_memory_128(RuntimeManager *rt_m, addr_t addr,
//                                                                uint128_t val);
#endif
...
#define MAKE_SIGNED_MEM_ACCESS(size) \
  ALWAYS_INLINE static int##size##_t __remill_read_memory_s##size(RuntimeManager *rt_m, \
                                                                  addr_t addr) { \
    return static_cast<int##size##_t>(__remill_read_memory_macro##size(rt_m, addr)); \
  } \
\
  ALWAYS_INLINE static void __remill_write_memory_s##size(RuntimeManager *rt_m, addr_t addr, \
                                                          int##size##_t val) { \
    __remill_write_memory_macro##size(rt_m, addr, static_cast<uint##size##_t>(val)); \
  }
...
#define MAKE_MREAD(size, ret_size, type_prefix, access_suffix) \
  ALWAYS_INLINE static type_prefix##ret_size##_t _Read(RuntimeManager *rt_m, \
                                                       Mn<type_prefix##size##_t> op) { \
    return __remill_read_memory_macro##access_suffix(rt_m, op.addr); \
  } \
\
  ALWAYS_INLINE static type_prefix##ret_size##_t _Read(RuntimeManager *rt_m, \
...
ALWAYS_INLINE static float80_t _Read(RuntimeManager *rt_m, Mn<float80_t> op) {
  return __remill_read_memory_macrof80(rt_m, op.addr);
}
...
template <typename T>
ALWAYS_INLINE static void _Write(RuntimeManager *rt_m, T &dst, T src) {
  dst = src;
}

// Make write operators for writing values to registers.
#define MAKE_RWRITE(type) \
  ALWAYS_INLINE static void _Write(RuntimeManager *rt_m, RnW<type> reg, type val) { \
    *(reg.val_ref) = val; \
  }
...
#define MAKE_MWRITE(size, write_size, mem_prefix, type_prefix, access_suffix) \
  ALWAYS_INLINE static void _Write(RuntimeManager *rt_m, MnW<mem_prefix##size##_t> op, \
                                   type_prefix##write_size##_t val) { \
    __remill_write_memory_macro##access_suffix(rt_m, op.addr, val); \
  }
...
#define MAKE_MREADV(prefix, size, vec_accessor, mem_accessor) \
  template <typename T> \
  ALWAYS_INLINE static auto _##prefix##ReadV##size(RuntimeManager *rt_m, MVn<T> mem) \
    ...
    _Pragma("unroll") for (addr_t i = 0; i < NumVectorElems(vec); ++i) { \
      vec.elems[i] = __remill_read_memory_macro##mem_accessor(rt_m, mem.addr + (i * el_size)); \
    } \
    return vec; \
  } \
\
  template <typename T> \
  ALWAYS_INLINE static auto _##prefix##ReadV##size(RuntimeManager *rt_m, MVnW<T> mem) \
      ->decltype(T().vec_accessor) { \
    decltype(T().vec_accessor) vec = {}; \
    const addr_t el_size = sizeof(vec.elems[0]); \
    _Pragma("unroll") for (addr_t i = 0; i < NumVectorElems(vec); ++i) { \
...
#define MAKE_WRITEV(prefix, size, accessor, kind, base_type) \
  template <typename T> \
  ALWAYS_INLINE static void _##prefix##WriteV##size(RuntimeManager *rt_m, kind<T> vec, \
                                                    base_type val) { \
...
#define MAKE_MWRITEV(prefix, size, vec_accessor, mem_accessor, base_type) \
  template <typename T> \
  ALWAYS_INLINE static void _##prefix##WriteV##size(RuntimeManager *rt_m, MVnW<T> mem, \
                                                    base_type val) { \
    T vec{}; \
    ...
    _Pragma("unroll") for (addr_t i = 0; i < NumVectorElems(vec.vec_accessor); ++i) { \
      __remill_write_memory_macro##mem_accessor(rt_m, mem.addr + (i * el_size), \
...
\
  ...
  ALWAYS_INLINE static void _##prefix##WriteV##size(RuntimeManager *rt_m, MVnW<T> mem, \
    ...
    const addr_t el_size = sizeof(base_type); \
    _Pragma("unroll") for (addr_t i = 0; i < NumVectorElems(val); ++i) { \
      __remill_write_memory_macro##mem_accessor(rt_m, mem.addr + (i * el_size), val.elems[i]); \
    } \
  }
...
#define MAKE_WRITE_REF(type) \
  ALWAYS_INLINE static void _Write(RuntimeManager *rt_m, type &ref, type val) { \
    ref = val; \
  }
...
// ALWAYS_INLINE static uint128_t __remill_read_memory_128(RuntimeManager *rt_m, addr_t addr) {
//   uint128_t low_qword = ZExt(__remill_read_memory_macro64(rt_m, addr));
//   uint128_t high_qword = ZExt(__remill_read_memory_macro64(rt_m, addr + 8));
...
//   __remill_write_memory_macro64(rt_m, addr, low_qword);
//   __remill_write_memory_macro64(rt_m, addr + 8, high_qword);
// }
#endif
